...
| TD-046 | Media | `src/presentation/households/screens/HouseholdHubScreen.tsx` | **El hub «Casa» puede ofrecer todo el sistema del hogar menos «gestionar miembros»: no existe pantalla.** El backend sí expone el contrato (`GET /households/:id/members`, consumido por `src/presentation/households/hooks/useHouseholdMembers.ts`) y ese hook ya se usa para resolver *nombres* de responsable en Tareas — pero **ninguna pantalla lo muestra** y no hay ninguna ruta de miembros en `RootStackParamList`. Consecuencia: invitar/quitar/ver miembros no es descubrible desde la app, y la composición del hogar solo se intuye por el selector. **No se inventa una pantalla** (requiere diseño de producto: qué se puede hacer desde ahí, y si AR/expulsar vive en Mobile o en web). Estado: **bloqueado por diseño** — es el único hueco del mapa de Casa |
| TD-047 | **Alta** | `tsconfig.json` (`lib` incluye `dom`), `src/presentation/*/screens/*.tsx` | **`navigation` sin destructurar de las props compila igual: resuelve al global `window.navigation` de la Navigation API del DOM.** Al añadir el botón «Volver» en 27 pantallas, tres (`MeowScreen`, `NotificationsListScreen`, `TrustedContactsScreen`) usaban `(_props: Props)` y `navigation.goBack()` **compilaba sin error** apuntando a `Navigation.back()` del DOM — el error solo apareció al ejecutar `tsc` en el gate (`TS2551: Did you mean 'back'?`). En dispositivo eso es un `TypeError` en tiempo de render: la pantalla no abre. Corregido en el mismo commit (destructurando `{ navigation }: Props`), pero **la causa raíz sigue viva**: con `lib: ["dom"]` cualquier identificador no definido tiene una segunda oportunidad de tipar bien por accidente. Fix real: separar `lib` del código de app (o `types` de RN) para que los globals del DOM no estén en el programa. Estado: **síntoma corregido, causa abierta** |
| TD-048 | Media | `src/core/navigation/MainTabs.tsx`, `FloatingTabBar.tsx` | **La arquitectura de experiencia está a medio camino: HOY, CASA y el `+` universal existen, pero `TÚ` y `DESCUBRIR` no, y la barra sigue exponiendo 5 módulos internos.** Medido en `MainTabs.tsx`: `Dashboard`, `TasksList`, `ShoppingLists`, `Expenses`, `Calendar` — cinco destinos de módulo como IA de primer nivel, más el botón central añadido. `Casa` alcanza 9 capacidades (Inventario, Momentos, Ahorros, Gastos recurrentes, Mascotas…) que **no caben en la barra**, así que siguen viviendo detrás de un hub. `TÚ` (perfil, cuenta, preferencias) no tiene entrada de primer nivel: `AccountScreen` se alcanza desde el chip del usuario en Hoy. Sin `DESCUBRIR` no hay dónde poner capacidades secundarias sin saturar Hoy/Casa. **No se toca en este checkpoint por decisión explícita**: rehacer el navigator es un cambio de producto y habría que reasignar 42 pantallas | Pendiente — decisión de producto: qué 4–5 destinos de primer nivel, y qué entra en `TÚ`/`DESCUBRIR` |
| TD-049 | Baja | `FloatingTabBar.tsx` (`TAB_LABELS`), `HouseholdHubScreen.tsx` (`GROUPS`) | **Una misma capacidad tiene dos nombres visibles: «Agenda» en la barra de pestañas y «Calendario» en el hub Casa.** Ambos son españoles y comprensibles, así que no es un problema de idioma sino de **consistencia**: un usuario que entra por «Agenda» y busca «Calendario» (o al revés) no tiene confirmación de que sean lo mismo. Mismo patrón potencial en otros pares barra↔hub que habrá que alinear al cerrar TD-048. **No se renombra aquí**: elegir el nombre definitivo es una decisión de producto, y cambiarlo solo en un sitio empeoraría la inconsistencia | Pendiente — decisión de producto: un nombre único por capacidad |
| TD-050 | Media | 10 pantallas bajo `src/presentation/` (Calendar, Expenses, Inventory, Meow, Moments, Pets, Savings, Shopping, Tasks + `HouseholdHubScreen`) | **El selector de hogar (`Personal | Familia Peluche`) sigue montado a tamaño protagonista en 10 pantallas.** `grep -rln HouseholdSelector src/presentation` da 9 consumidores más el propio hub. Es la primera pieza visual de cada pantalla, por encima del contenido, aunque en la mayoría de pantallas el hogar activo **no cambia lo que se muestra** (p. ej. `TasksListScreen` lista tareas personales — medido en el checkpoint anterior). El `+ Crear hogar` repetido ya se eliminó (vive en Casa), pero el patrón «selector grande en todas las screens» no se ha sustituido todavía por el **patrón contextual compacto** que pide el brief. **No se toca en este checkpoint**: son 10 pantallas fuera del slice autorizado (Hoy, Casa, `+`, Tareas) | Pendiente — requiere definir la forma compacta (¿chip en la cabecera? ¿filtro?) antes de propagarla |
...
Precisión (2026-09-18): esta medición es de **tokens cromáticos sobre `surface`**, y ahí el tema oscuro
efectivamente pasa. **No cubre el par `text` sobre los `*Soft`**, que en oscuro sí falla y de forma
ilegible (1.00:1), porque `colors.dark` no redefine esos tokens — ver **TD-044**.
...
El problema de **este** par es exclusivo del tema claro: los pasteles se eligieron para leer sobre
oscuro, y en claro falta la otra mitad. (El tema oscuro tiene un defecto propio y distinto: TD-044.)
...
- **TD-045** es la mitad geométrica del «GAP 9 — icono» (el informe de diseño lo llama GAP 12) y
  ...
  *squircle*, que recorta bastante menos que un círculo, así que el 74.12% actual puede no ser un
  ...
  que no existe.
- **El checkpoint de navegación (2026-09-18) es la contraparte de TD-044: la premisa del hallazgo
  también estaba a medias.** El encargo decía «inventory no se encuentra»; la medición encontró algo
  más grande — **Inventario no era un caso especial, era el síntoma**. Estaba registrado y tipado
  pero con **cero entradas**: ninguna llamada del árbol lo navegaba. Y el mismo patrón apareció dos
  veces más, medido: **Gastos recurrentes** solo era alcanzable desde el fondo del tab Gastos (lo
  enlaza `ExpensesScreen`), y **27 pantallas empujadas** — todas las de superficie de app, es decir
  las 42 pantallas enrutadas menos las 10 de auth y las 5 tabs — **no tenían cabecera ni botón de
  volver**. La causa raíz es de sistema, no de pantalla: `RootNavigator` oculta el header nativo
  (`headerShown: false`), así que **cada pantalla empujada es responsable de su propia salida**. De
  ahí que la corrección sea una primitiva (`ScreenHeader`) aplicada a las 27, y no 27 botones.
  Estado tras el bloque: **ninguna pantalla registrada queda sin entrada, y ninguna empujada sin
  salida** (ambas cosas comprobadas mecánicamente sobre el grafo de navegación, no a ojo). Detalle
  importante del método: el `tsc` del gate cazó **tres pantallas cuyo `navigation` no estaba
  destructurado y compilaba igual** — ver **TD-047**, el hallazgo más peligroso del bloque, porque
  era un `TypeError` en dispositivo invisible para lint y para los tests (el mock sí tenía `goBack`).
- **TD-046 y TD-047** nacieron de este checkpoint. **TD-046** es el único hueco del mapa de Casa
  (gestionar miembros: contrato y hook existen, pantalla no) y queda **bloqueado por diseño**.
  **TD-047** es la causa raíz del fallo de `navigation`: con `lib: ["dom"]` en el programa de
  TypeScript, un identificador no definido **resuelve al global `window.navigation`** y el error solo
  aparece al ejecutar el gate, no en el editor.
